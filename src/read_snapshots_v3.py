...
### Version 3 is only to read in ###
### Disk positions & velocities ###
### also changed 
...
parser.add_argument('--N',type = int, default=200, # in homoI and homoII, the value is 10
...
for filename in all_files:
    ...
    for grp in list(f.keys()):
        if grp == 'PartType2': ###Change from version 1 ###
            type_N = f['{}'.format(grp)]
            ...
            #density distribution
            axes[1,1].plot(r_dens,dens,color='k')
            #velocity dispersion
            axes[0,2].plot(r_disp,v_disp,color='k')
    ...
    axes[1,2].axis('off')
    axes[1,2].scatter([],[],c='k',label='Disk')
    
    axes[1,2].legend()
    plt.tight_layout()
    plt.savefig('../previous_attempts/attempt{}/{}_results/plots_v3/snap{}_pos.png'.format(args.attemptN,args.combo,filename[-10:-5]))
    plt.close(fig)
